var t2 = new SplitText('.role').chars,
    color2 = '#17c0fd',
    color1 = '#fff',
    moveBar = ()=>{ gsap.set('.bar', {left:gsap.getProperty('.welcome','width') + 1}) };

gsap.timeline({delay:0.2})
    .set('.welcome',   {color:color1, fontWeight:'regular'})
    .set('.role',   {color:color2, fontWeight:'bold', opacity:0, x:gsap.getProperty('.welcome','width')-2, immediateRender:true})
    ...
    .from('.welcome',  {duration:1.1, width:0, ease:SteppedEase.config(14), onUpdate:moveBar}, 2.5)
    ...
    .to('.role',    {duration:0.01, opacity:1}, '-=0.1')
    ...
    .to('.welcome',    {duration:1.5, opacity:0.25, ease:Power3.easeInOut}, '-=1.2')
    .timeScale(1.45)